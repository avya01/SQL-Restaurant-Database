...
(43, 'Leena Nuckchady', 574839029, 16),
...
--QUERy TO SELECT ALL THE CUSTOMERS WHO HAS ORDERED FROM THE RESTAURANT TROPICAL BITES
...
--ACP
--QUERY TO SELECT HOW MANY CUSTOMERS HAS ORDERED FOOD FROM EACH RESTAURANTS (Couldn't accomplish it, sir, please provide guidance on the counting part.)

SELECT RESTO_NAME, CUSTOMERS.CUST_ID
FROM RESTAURANTS
JOIN CUSTOMERS
ON RESTAURANTS.RESTO_ID = CUSTOMERS.RESTO_ID;

--QUERY TO SELECT THE BEST AND THE WORST RATED RESTAURANT

--BEST
SELECT RESTO_NAME, MAX(RATING) FROM RESTAURANTS;

--WORST
SELECT RESTO_NAME, MIN(RATING) FROM RESTAURANTS;

--QUERY TO SELECT WHAT IS THE AVERAGE RATING OF ALL THE RESTAURANTS

SELECT AVG(RATING) AS "Average rating of the restaurants" 
FROM RESTAURANTS;
...
--RESTO_ID 37
SELECT RESTO_NAME, CUST_NAME
FROM CUSTOMERS
JOIN RESTAURANTS
ON CUSTOMERS.RESTO_ID = RESTAURANTS.RESTO_ID
WHERE CUSTOMERS.RESTO_ID = 37;
